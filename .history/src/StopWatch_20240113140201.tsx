...
const StopWatch = () => {
  ...
  const handleLap = () => {
    if (isRunning) {
      // Calculate lap time based on the difference between current time and lap start time
      const lapTime = timeElapsed - lapStartTime;

      // Update lap start time for the next lap
      lapStartTime = timeElapsed;

      const newLap = { number: laps.length + 1, time: lapTime };
      ...
      // Update the state with the modified laps array
      setLaps(newLaps);
    }
  };
