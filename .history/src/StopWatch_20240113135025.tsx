...
const StopWatch = () => {
  ...
  const handleLap = () => {
    ...
    const lapTime = isRunning ? timeElapsed : 0; // Get the current time when lap is taken
    const newLapEntry = lapTime - (laps.length > 0 ? laps[laps.length - 1] : 0); // Calculate lap time relative to the previous lap
    setLaps([...laps, newLapEntry]);
  };
  const handleReset = () => {
    setIsRunning(false);
    setTimeElapsed(0);
