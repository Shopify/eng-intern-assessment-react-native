...
const StopWatch = () => {
  ...
  const handleLap = () => {
    // Calculate the lap time by subtracting the previous lap time (if available) from the current total time
    const lapTime = laps.length > 0 ? timeElapsed - laps[laps.length - 1] : timeElapsed;
  
    if (laps.length >= 10) {
      // If there are already 10 laps, remove the oldest lap (the last element)
      const newLaps = [lapTime, ...laps.slice(0, -1)];
      setLaps(newLaps);
    } else {
      // If there are fewer than 10 laps, just prepend the new lap time to the existing laps
